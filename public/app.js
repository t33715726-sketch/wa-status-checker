...
(function () {
  ...
  $('contactsFile').addEventListener('change', function () {
    ...
    if (file.size > MAX_FILE_BYTES) {
      contactsError('הקובץ גדול מדי. אם ייצאת מאייפון עם תמונות אנשי הקשר, ייצא שוב בלי תמונות - contacts.google.com ← ייצוא ← vCard.');
      return;
    }
    ...
    reader.onload = function () {
      ...
      // רצפה מוחלטת, לפני השער האחוזי: אפילו שדה טלפון אחד שלא נקרא
      // הוא אדם אחד שעלול להידרס. שער באחוזים מפספס בדיוק את זה.
      if (parsed.unparsed > 0) {
        contactsError(
          'זוהו ' + parsed.unparsed + ' שדות טלפון שלא הצלחנו לקרוא. ' +
          'אי אפשר להמשיך, כי בדיוק הם עלולים להידרס בייבוא. ' +
          'שלח לי את הקובץ ונתקן את הקריאה.'
        );
        return;
      }
